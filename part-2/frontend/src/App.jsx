...
function App() {
    ...
    const timer = setInterval(() => {
      setMessages((prev) => {
        ...
        if (prev.length >= currentLimit) {
          console.log(
            `⏸️ Reached conversation limit of ${currentLimit} messages. Pausing.`
          );
          setIsPlaying(false);
          return prev;
        }
  ...
  useEffect(() => {
    ...
    // Use 5000ms for the first message, 3000ms for subsequent messages
    const intervalTime = messages.length === 1 ? 5000 : 3000;
    ...
    <div className="app-container">
      ...
      <div className="controls">
        <button
          ...
          disabled={loading || (useBackend && !conversationId)}
        >
          {isPlaying
            ? '⏸ Pause'
      ...
      <div className="stats">
        <span>
          ...
          {messages.length >= currentLimit &&
            '⏸️ (Limit Reached - Click Resume for +10)'}
          {useBackend &&
            conversationId &&
